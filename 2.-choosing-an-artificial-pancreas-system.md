...
* Loop has a different algorithm than OpenAPS, also works with older Medtronic pump models, as well as the Eros OmniPod, can be used with most CGMs, uses a separate radio device \(called a RileyLink\) to bridge communication to an iOS device, and uses an iOS device \(usually an iPhone\) as its controller.
